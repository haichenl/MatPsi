...
double myHF::compute_energy(SharedMatrix EnvMat)
{
    ...
    if(EnvMat->nirrep() != 1 || EnvMat->nrow() != basisset_->nbf() || EnvMat->ncol() != basisset_->nbf())
        throw PSIEXCEPTION("myHF::compute_energy(EnvMat): Environment potential energy matrix dimensions do not agree.");
