...
public:
    myRHF(Options& options, boost::shared_ptr<PSIO> psio, boost::shared_ptr<Chkpt> chkpt);
    myRHF(Options& options, boost::shared_ptr<PSIO> psio);
    virtual ~myRHF();
    ...
    SharedMatrix J() { return J_; }
    SharedMatrix K() { return K_; }
...
myRHF::myRHF(Options& options, boost::shared_ptr<PSIO> psio, boost::shared_ptr<Chkpt> chkpt)
    : myHF(options, psio, chkpt)
{
    common_init();
}

myRHF::myRHF(Options& options, boost::shared_ptr<PSIO> psio)
    : myHF(options, psio)
{
    common_init();
}
...
void myRHF::common_init()
{
    if (multiplicity_ != 1) throw PSIEXCEPTION("myRHF: myRHF reference is only for singlets.");
    Drms_ = 0.0;
...
SharedMatrix myRHF::Da() const
{
    return D_;
}

void myRHF::save_density_and_energy()
{
...
void myRHF::save_information()
{
}

void myRHF::compute_orbital_gradient(bool save_fock)
{
    ...
    if(save_fock){
        if (initialized_diis_manager_ == false) {
            if (scf_type_ == "direct")
                diis_manager_ = boost::shared_ptr<DIISManager>(new DIISManager(max_diis_vectors_, "myHF DIIS vector", DIISManager::LargestError, DIISManager::InCore));
            else
                diis_manager_ = boost::shared_ptr<DIISManager>(new DIISManager(max_diis_vectors_, "myHF DIIS vector", DIISManager::LargestError, DIISManager::OnDisk));
...
void myRHF::form_D()
{
    ...
    if (debug_) {
        fprintf(outfile, "in myRHF::form_D:\n");
        D_->print();
    }
}

void myRHF::damp_update()
{
    for(int h = 0; h < nirrep_; ++h){
...
double myRHF::compute_initial_E()
{
    double Etotal = nuclearrep_ + D_->vector_dot(H_);
    return Etotal;
}

double myRHF::compute_E()
{
...
void myRHF::save_sapt_info()
{
    ...
    if (soccpi_[0] != 0)
    {
        fprintf(outfile,"Aren't we in myRHF Here? Pair those electrons up cracker!\n"); fflush(outfile);
        abort();
    }
    ...
    sprintf(key_buffer,"%s myHF Energy",body_type);
    ...
    sprintf(key_buffer,"%s myHF Eigenvalues",body_type);
    ...
    sprintf(key_buffer,"%s myHF Coefficients",body_type);
...
#define ID(x) ints.DPD_ID(x)
        ...
        global_dpd_->buf4_scmcopy(&I, PSIF_LIBTRANS_DPD, "myRHF Singlet Hessian (IA|JB)", 4.0);
        // Triplet A_ia_jb = -(ib|ja)
        global_dpd_->buf4_sort_axpy(&I, PSIF_LIBTRANS_DPD, psrq,
                           ID("[O,V]"), ID("[O,V]"), "myRHF Triplet Hessian (IA|JB)", -1.0);
        ...
        global_dpd_->buf4_init(&Atrip, PSIF_LIBTRANS_DPD, 0, ID("[O,V]"), ID("[O,V]"),
                      ID("[O,V]"), ID("[O,V]"), 0, "myRHF Triplet Hessian (IA|JB)");
        ...
        global_dpd_->buf4_init(&Asing, PSIF_LIBTRANS_DPD, 0, ID("[O,V]"), ID("[O,V]"),
                      ID("[O,V]"), ID("[O,V]"), 0, "myRHF Singlet Hessian (IA|JB)");
        ...
        fprintf(outfile, "\tLowest singlet (myRHF->myRHF) stability eigenvalues:-\n");
        print_stability_analysis(singlet_eval_sym);
        fprintf(outfile, "\tLowest triplet (myRHF->UHF) stability eigenvalues:-\n");
